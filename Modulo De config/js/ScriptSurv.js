let questions = [];
let questionsImg = [];
let defaultQuestionsData = []; // To store the original default questions with their IDs and status
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const guardarPreguntasSugeridasBtn = document.getElementById('GuardarPreguntasSugeridasBtn');

    guardarPreguntasSugeridasBtn.addEventListener('click', (event) => {
...
function loadDefaultQuestions(studyId) {
    const url = `https://api.cheetah-research.ai/configuration/getDefaultQuestions/${studyId}/`;
    axios.get(url)
        .then(response => {
            defaultQuestionsData = response.data.questions; // Store the full data
            const defaultQuestionsContainer = document.getElementById('defaultQuestionsContainer');
            defaultQuestionsContainer.innerHTML = ''; // Clear previous questions
            
            if (!defaultQuestionsData || defaultQuestionsData.length === 0) {
                // console.log("No se encontraron preguntas sugeridas desde la API.");
                return;
            }

            defaultQuestionsData.forEach((questionData, index) => {
                const newListItem = document.createElement('div');
                newListItem.classList.add('list-group-item', 'list-group-item-action', 'align-items-start');
                newListItem.style.fontFamily = "hedliner";
                newListItem.style.display = 'flex';
                ...
                topRowDiv.classList.add('d-flex', 'w-100', 'justify-content-between', 'align-items-center');

                const newH5 = document.createElement('h5');
                newH5.classList.add('mb-1');
                newH5.style.fontFamily = "IBM Plex Sans";
                newH5.style.flexGrow = '1'; // Allow question text to take available space
                let unescapedQuestion = questionData.question.replace(/\\n/g, '\n');
                let processedQuestion = unescapedQuestion.replace(/\n/g, '<br>');
                newH5.innerHTML = processedQuestion;
                newH5.setAttribute('data-id', questionData.id); // Store question ID

                const statusSelect = document.createElement('select');
                statusSelect.classList.add('form-select', 'form-select-sm');
                statusSelect.style.width = 'auto'; // Adjust width to content
                statusSelect.style.marginLeft = '10px';

                const optionActive = document.createElement('option');
                optionActive.value = "1";
                optionActive.textContent = "Activada";
                const optionInactive = document.createElement('option');
                optionInactive.value = "0";
                optionInactive.textContent = "Desactivada";

                statusSelect.appendChild(optionActive);
                statusSelect.appendChild(optionInactive);
                statusSelect.value = questionData.status.toString(); // Set current status

                statusSelect.addEventListener('change', (event) => {
                    const newStatus = parseInt(event.target.value, 10);
                    const questionToUpdate = defaultQuestionsData.find(q => q.id === questionData.id);
                    if (questionToUpdate) {
                        questionToUpdate.status = newStatus;
                    }
                });

                topRowDiv.appendChild(newH5);
                topRowDiv.appendChild(statusSelect);
                ...
                editBtn.addEventListener('click', (event) => {
                    event.preventDefault();
                    const overlay = document.getElementById('overlay');
                    // Store current question text to revert if textarea is cleared and saved
                    const currentQuestionTextInH5 = questionData.question;

                    overlay.innerHTML = `
                        <div id="overlayContent">
                            <textarea id="EditDefaultPreguntaTXT" class="form-control" name="Nombre" placeholder="Ingresa tu pregunta" style="width: 100%; font-family: IBM Plex Sans; margin-bottom: 5px; min-height: 100px;">${currentQuestionTextInH5.replace(/\\n/g, '\n')}</textarea>
                            <button id="GuardarDefaultEdit" class="btn btn-primary" style="margin: 10px 10px 0 0;font-family: hedliner; margin-bottom: 5px;color: var(--bs-CR-gray); background-color: var(--bs-CR-orange);">Guardar</button>
                            <button id="CerrarOverlay" class="btn btn-secondary" style="margin: 10px 0 0 0;font-family: hedliner; margin-bottom: 5px;">Cerrar</button>
                        </div>
                    `;
                    overlay.style.display = 'flex';
                    ...
                    document.getElementById('GuardarDefaultEdit').addEventListener('click', () => {
                        const editedQuestionText = document.getElementById('EditDefaultPreguntaTXT').value;
                        if (editedQuestionText.trim()) {
                            let unescapedEditedQuestion = editedQuestionText.replace(/\\n/g, '\n');
                            let processedEditedQuestion = unescapedEditedQuestion.replace(/\n/g, '<br>');
                            newH5.innerHTML = processedEditedQuestion;
                            
                            const originalQuestion = defaultQuestionsData.find(q => q.id === questionData.id);
                            if (originalQuestion) {
                                originalQuestion.question = editedQuestionText; // Store with original newlines (escaped by user or not)
                            }
                            overlay.style.display = 'none';
                        } else {
                ...
                newListItem.appendChild(topRowDiv);
                newListItem.appendChild(buttonsDiv);
                defaultQuestionsContainer.appendChild(newListItem);
            });
        })
        .catch(error => {
            console.error('Error al cargar las preguntas sugeridas:', error);
            // Optionally, display a user-friendly message in the UI
            const defaultQuestionsContainer = document.getElementById('defaultQuestionsContainer');
            defaultQuestionsContainer.innerHTML = '<p class="text-danger">Error al cargar las preguntas sugeridas. Intente recargar la página.</p>';
        });
}

function saveDefaultQuestions() {
    const studyId = localStorage.getItem('selectedStudyId');
    const questionsToSave = defaultQuestionsData
        .filter(q => q.status === 1) // Ensure we only send active questions that were displayed
        .map(q => q.question); // Extract the text of the questions

    if (questionsToSave.length === 0) {
        // console.log("No hay preguntas sugeridas activas para guardar.");
        // location.reload(); // Reload even if no questions, to reflect any state changes if necessary
        return; // Or handle as an error/notification
    }

    const payload = {
        suggested_questions: questionsToSave
    };

    const url = `https://api.cheetah-research.ai/configuration/updateDefaultQuestIons/${studyId}/`; // Corrected API endpoint
    ...
    .then(response => {
        // console.log('Preguntas sugeridas guardadas:', response.data);
        location.reload(); // Recargar la página inmediatamente
    })
    .catch(error => {
        console.error('Error al guardar las preguntas sugeridas:', error);
        alert('Error al guardar las preguntas sugeridas. Por favor, inténtalo de nuevo.');
    });
}